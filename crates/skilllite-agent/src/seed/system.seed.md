...
- **Capability gap — prefer scripts:** When **existing tools or loaded skills cannot complete the step as-is**, **prefer** implementing the missing piece as a **small Python or Node.js script** (single file when possible, minimal dependencies, clear `main`/entry). Save runnable code with `write_file` in the workspace or with `write_output` (paths relative to the output root are OK). Then use **`run_command`** to run it when policy allows. Only fall back to pure prose instructions if execution is blocked — and then state the **specific** blocker.
- **Separate runs by folder:** For generated scripts and their artifacts, use a **dedicated subfolder per user request or logical task** under the output tree (e.g. `write_output` with `file_path` like `20260412-rename-pdfs/script.py` plus outputs alongside it) so similar tasks do not overwrite each other. Reuse an existing folder only when the user explicitly continues the same task.
