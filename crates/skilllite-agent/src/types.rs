...
pub struct PlanningRule {
    ...
    /// Whether this rule can be modified by the evolution engine.
    /// `false` = CORE (seed), `true` = MUTABLE (evolved/user).
    #[serde(default)]
    pub mutable: bool,
    /// Origin of this rule: "seed", "evolved", or "user".
    #[serde(default = "default_origin")]
    pub origin: String,
    /// Whether this rule has proven effective enough for direct reuse
    /// (simplified Capsules: skip LLM re-derivation for known-good rules).
    #[serde(default)]
    pub reusable: bool,
    /// Effectiveness score (0.0-1.0), -1.0 = insufficient samples.
    /// Computed from decisions JOIN decision_rules.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub effectiveness: Option<f32>,
    /// Number of times this rule was triggered (used for reusable promotion).
    #[serde(default, skip_serializing_if = "Option::is_none")]
