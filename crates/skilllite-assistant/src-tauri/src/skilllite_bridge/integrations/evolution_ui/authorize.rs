...
use crate::skilllite_bridge::paths::{
    find_project_root, load_dotenv_for_child, with_explicit_workspace_env,
};

fn background_run_args(workspace: &str) -> Vec<String> {
    vec![
        "evolution".to_string(),
        "run".to_string(),
        "--json".to_string(),
...
pub fn authorize_capability_evolution(
    ...
    std::thread::spawn(move || {
        ...
        cmd.args(background_run_args(&workspace_owned))
            .current_dir(&root)
            .stdout(std::process::Stdio::piped())
            .stderr(std::process::Stdio::piped());
        for (k, v) in with_explicit_workspace_env(load_dotenv_for_child(&workspace_owned), &root) {
            cmd.env(k, v);
        }
...
mod tests {
    ...
    #[test]
    fn background_run_args_include_explicit_workspace() {
        assert_eq!(
            background_run_args("/tmp/project"),
            vec![
                "evolution",
                "run",
                "--json",
