...
export const zhMessages: Record<string, string> = {
  ...
  "settings.environment.intro":
    "「运行时」指 Python 与 Node，供沙箱执行需要脚本的技能；「Git」用于从 GitHub 等地址通过 skilllite 克隆安装技能。使用 ClawHub 或仅本地技能时，可以不安装 Git。",
  ...
  "settings.environment.gitDesc": "在终端使用 `skilllite add <仓库地址>` 时，需要本机 PATH 中可执行 `git`。",
