...
- **IDE 三栏**：顶栏 **IDE** 或 **设置 → 工作区与沙箱 → IDE 三栏主界面** 可切换为「工作区文件树 | 编辑器 | 对话」；左侧 **会话** 标签仍打开原会话列表。大目录会跳过 `node_modules`、`target` 等；敏感路径与写入规则一致（如 `.env` 不可读/写）。关闭 IDE 布局后恢复右侧状态栏。聊天里 **`read_file` 工具结果**的 **「在 IDE 中打开」** 会自动开启 IDE 布局并在中间栏从磁盘打开该文件（无工具路径时仍可用「全屏查看 / 编辑」弹窗）。
