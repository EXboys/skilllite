...
/// When SKILLBOX_QUIET=1, only WARN and above are logged.
pub fn init_tracing() {
    let level: String = if is_quiet() {
        "skillbox=warn".to_string()
    } else {
        env::var("SKILLBOX_LOG_LEVEL").unwrap_or_else(|_| "skillbox=info".to_string())
    };

    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new(level));

    let json = env::var("SKILLBOX_LOG_JSON")
