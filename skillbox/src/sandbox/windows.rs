...
/// Check if skillbox is installed in WSL
fn is_skillbox_in_wsl() -> bool {
    let output = Command::new("wsl")
        .args(["-e", "which", "skillbox"])
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .output();
...
/// Execute skill via WSL2 using the Linux skillbox binary
fn execute_via_wsl2(
    skill_dir: &Path,
    env_path: &Path,
    metadata: &SkillMetadata,
    ...
    // Build the skillbox command for WSL
    let mut args = vec![
        "-e".to_string(),
        "skillbox".to_string(),
        "run".to_string(),
    ...
    let output = Command::new("wsl")
        ...
        .stderr(Stdio::piped())
        .output()
        .context("Failed to execute skillbox via WSL")?;
...
pub fn diagnose_wsl() -> String {
    ...
    // Check skillbox in WSL
    let skillbox_available = is_skillbox_in_wsl();
    report.push_str(&format!("skillbox in WSL: {}\n", if skillbox_available { "Yes" } else { "No" }));

    if !skillbox_available {
        report.push_str("\nTo install skillbox in WSL:\n");
        ...
        report.push_str("  1. cd /mnt/c/path/to/skilllite/skillbox\n");
