...
///
/// This is the entry point for `skillbox mcp-serve`.
...
fn handle_list_skills(server: &McpServer) -> Result<String> {
    ...
    if !skills_dir.exists() {
        return Ok("No skills directory found. Use `skillbox add` to install skills.".to_string());
    }
    ...
    if skills.is_empty() {
        return Ok("No skills installed. Use `skillbox add` to install skills.".to_string());
    }
