...
def get_install_dir() -> Path:
    """
    Get the installation directory for the skilllite binary.
    
    Returns:
        Path to ~/.skilllite/bin/
    """
...
def get_download_url(version: Optional[str] = None) -> str:
    ...
    # Binary naming convention: skilllite-{platform}.tar.gz
    filename = f"skilllite-{plat}.tar.gz"
...
def is_installed() -> bool:
    """
    Check if the skilllite binary is installed.
...
def install(
    ...
    if not force and is_installed() and not needs_update(version):
        installed_version = get_installed_version()
        logger.info(f"✓ skilllite v{installed_version} is already installed")
        return get_binary_path()
    
    plat = get_platform()
    logger.info(f"Installing skilllite v{version} for {plat}...")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        archive_name = f"skilllite-{plat}.tar.gz"
    ...
    logger.info(f"✓ Successfully installed skilllite v{version}")
...
def uninstall() -> bool:
    """
    Uninstall the skilllite binary.
...
def find_binary() -> Optional[str]:
    ...
    Search order:
    1. SKILLLITE_BINARY_PATH / SKILLLITE_PATH (explicit override)
    2. ~/.cargo/bin/skilllite (cargo install - preferred when developing)
    3. ~/.skilllite/bin/skilllite (skilllite install - prebuilt)
    4. System PATH
    ...
    # 2. skilllite install prebuilt (~/.skilllite/bin)
    if found is None:
...
def check_binary_version(binary_path: str) -> Optional[str]:
    ...
    try:
        ...
        # clap outputs "skilllite 0.1.0\n"
        version_line = result.stdout.strip()
        # Extract version number from "skilllite X.Y.Z"
        parts = version_line.split()
        binary_version = parts[-1] if parts else version_line

        if binary_version != BINARY_VERSION:
            warnings.warn(
                f"skilllite binary version mismatch: "
            ...
            logger.warning(
                "skilllite version mismatch: binary=%s, SDK=%s, path=%s",
                binary_version, BINARY_VERSION, binary_path,
            )
        else:
            logger.debug(
                "skilllite version OK: %s (path=%s)", binary_version, binary_path,
            )
...
def ensure_installed(
    ...
    raise FileNotFoundError(
        f"skilllite binary not found. Searched locations:\n"
        f"{paths_info}\n\n"
        f"Install it with:\n"
        f"  skilllite install\n"
        f"Or build from source:\n"
        f"  cd skilllite && cargo build --release\n"
        f"  cd skilllite && cargo build          # (debug build also works)"
    )
