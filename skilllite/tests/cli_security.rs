...
fn security_scan_json_output() {
    ...
    let out = run_in_dir(&["security-scan", script_name, "--json"], tmp.path());
    assert!(
        out.status.success(),
...
fn security_scan_with_allow_network() {
    ...
    assert!(out.status.success(), "failed. stderr: {}", stderr_str(&out));
    let text = stdout_str(&out);
...
fn security_scan_nonexistent_file_fails() {
    let tmp = tempfile::tempdir().unwrap();
    let out = run_in_dir(&["security-scan", "nonexistent_script_xyz.py"], tmp.path());
