...
顶栏 **IDE** 或**设置 → 工作区与沙箱**可开启**IDE 三栏主界面**（工作区文件树、编辑器、对话）；**三栏之间的竖线可拖拽**调宽（会记住宽度）。关闭后恢复右侧状态栏。`read_file` 工具结果在对话中以约 **5 行可滚动**预览展示，**点击预览**可在已知路径时在 IDE 中打开该文件，否则打开全屏阅读；`list_directory` 为同样高度的可滚动目录树。详情见助手目录内 README。
