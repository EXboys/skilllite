...
> **说明**：本文档与根目录 `Cargo.toml` 中 `[workspace.package]` 版本一致（当前 **v0.1.25**）。Rust 为 Cargo workspace 多 crate；Python SDK 为薄桥接层（`python-sdk/skilllite/` 下约 770 行），导出 `scan_code`、`execute_code`、`chat`、`run_skill`、`get_binary`、`artifact_put`、`artifact_get` 等。
>
...
│   ├── pyproject.toml             # 包配置 (v0.1.25, 零运行时依赖)
│   └── skilllite/
