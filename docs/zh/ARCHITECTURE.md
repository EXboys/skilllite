...
> **说明**：本文档与根目录 `Cargo.toml` 中 `[workspace.package]` 版本一致（当前 **v0.1.20**）。Rust 为 Cargo workspace 多 crate；Python SDK 为薄桥接层（约 630 行），主要导出 `scan_code`、`execute_code`、`chat`、`run_skill`、`get_binary`。
>
...
│   ├── pyproject.toml             # 包配置 (v0.1.20, 零运行时依赖)
│   └── skilllite/
