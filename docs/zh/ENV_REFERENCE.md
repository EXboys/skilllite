...
| `SKILLLITE_COMPACTION_KEEP_RECENT` | int | `10` | 压缩后保留的最近 user/assistant 消息数。这些消息会写回压缩标记之后，供后续回合继续使用（不只留在当次压缩的内存返回值里）。 |
