...
**Evolution triggers (A9)**: **Periodic** (`SKILLLITE_EVOLUTION_INTERVAL_SECS`, default 30 min) **and** **unprocessed decision count** (≥ `SKILLLITE_EVOLUTION_DECISION_THRESHOLD`, default 10) spawn `skilllite evolution run`. **`ChatSession`** (`skilllite chat` / `agent-rpc`) **records decisions/metrics only** and does **not** run evolution in-process; **SkillLite Assistant** uses **Life Pulse** with the same env keys from the workspace `.env`. Headless CLI: run `skilllite evolution run` manually or via cron. In-chat P7 “authorize evolution” bubbles are not shown.
