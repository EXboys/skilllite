...
Sandbox-related variables are read through the **config layer** (`SandboxEnvConfig::from_env()`): runner, linux, macos, windows, and policy all read via config; `SKILLLITE_*` is preferred, `SKILLBOX_*` is a deprecated alias.

| Variable | Type | Default | Description |
|----------|------|---------|-------------|
| `SKILLLITE_SANDBOX_LEVEL` | int | `3` | **Recommended**. Sandbox level (1/2/3); alias `SKILLBOX_SANDBOX_LEVEL` |
| `SKILLLITE_NO_SANDBOX` | bool | `false` | Disable sandbox (not recommended); alias `SKILLBOX_NO_SANDBOX` |
| `SKILLLITE_ALLOW_PLAYWRIGHT` | bool | `false` | Relax sandbox for Playwright Skills; alias `SKILLBOX_ALLOW_PLAYWRIGHT` |
| `SKILLLITE_AUTO_APPROVE` | bool | `false` | **Recommended**. Auto-approve L3 prompts (not recommended); alias `SKILLBOX_AUTO_APPROVE` |
| `SKILLLITE_SCRIPT_ARGS` | string | - | Extra args passed to scripts; alias `SKILLBOX_SCRIPT_ARGS` |
...
**Usage**:
- When sandbox is unavailable: `SKILLLITE_SANDBOX_LEVEL=1` (or `SKILLBOX_SANDBOX_LEVEL=1` for compat)
...
Sandbox resource limits are read through **config** (`SandboxEnvConfig`); `SKILLLITE_*` and `SKILLBOX_*` are both supported.

| Variable | Type | Default | Description |
|----------|------|---------|-------------|
| `SKILLLITE_TIMEOUT_SECS` | int | `30` | **Recommended**. Sandbox execution timeout (seconds); alias `SKILLBOX_TIMEOUT_SECS` |
| `SKILLLITE_MAX_MEMORY_MB` | int | `256` | **Recommended**. Sandbox max memory (MB); alias `SKILLBOX_MAX_MEMORY_MB` |
