...
**Evolution triggers (A9)**: **SkillLite Assistant (desktop)** runs `skilllite evolution run` from Life Pulse when either `SKILLLITE_EVOLUTION_INTERVAL_SECS` (default 30 min) elapses **or** unprocessed decisions ≥ `SKILLLITE_EVOLUTION_DECISION_THRESHOLD` (default 10). `skilllite chat` / the agent subprocess **records decisions/metrics only** and does not start in-process evolution; use manual `skilllite evolution run` or an external scheduler for CLI-only workflows.
