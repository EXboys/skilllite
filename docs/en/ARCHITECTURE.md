...
> **Note**: This document matches the root `Cargo.toml` `[workspace.package]` version (currently **v0.1.24**). Rust uses a Cargo workspace with modules split into separate crates; the Python SDK is a thin bridge layer (~770 lines of Python under `python-sdk/skilllite/`) exporting `scan_code`, `execute_code`, `chat`, `run_skill`, `get_binary`, `artifact_put`, `artifact_get`, and helpers.
>
...
│   ├── pyproject.toml             # Package config (v0.1.24, zero runtime deps)
│   └── skilllite/
